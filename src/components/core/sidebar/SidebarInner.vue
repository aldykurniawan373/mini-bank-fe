<script setup lang="ts">
  ...
  const menus = computed(() => {
    return Object.entries(APP_MENU)
      .map(([key, value]) => ({
        key,
        name: value.name,
        routes: value.routes
          .filter((r) => {
            ...
            if (r.path === 'users' && !authStore.isPimpinan) {
              return false;
            }
            return !r.hidden;
          })
          .map((r) => ({
            ...r,
            active: r.path.startsWith('/') 
              ? r.path === route.path 
              : `/dashboard/${r.path}` === route.path,
          })),
      }))
      .filter((menu) => menu.routes.length > 0); // Hapus menu kosong
  });
